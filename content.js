...
function isSentenceSensitiveLanguage(langCode) {
  const prefix = getLanguagePrefix(langCode);
  return prefix === 'de' || prefix === 'ja';
}

function needsCompleteAutoSentence() {
  return isSentenceSensitiveLanguage(currentSourceLang) ||
    isSentenceSensitiveLanguage(currentSettings.lang);
}
...
// Keep automatic captions paired with their translation whenever possible.
// The hint is only a bounded fallback for slow/failed translation requests.
const LIVE_ASR_TRANSLATION_WAIT_MS = 800;
const AUTO_FILE_TRANSLATION_WAIT_MS = 1000;
const YOUTUBE_TRANSLATION_WARMUP_MS = 300;
const INITIAL_TRANSLATION_LOOKAHEAD = 6;
const AUTO_INITIAL_TRANSLATION_LOOKAHEAD = 8;
const AUTO_FILE_WARMUP_LOOKAHEAD_SECONDS = 10;
const AUTO_FILE_WARMUP_TIMEOUT_MS = 1500;
const PRELOAD_BATCH_DELAY_MS = 80;
const DOM_TRANSLATION_DEBOUNCE_MS = 80;
const LIVE_ASR_COMMIT_STABILITY_MS = 450;
const LIVE_ASR_SENTENCE_FINALIZE_MS = 850;
const LIVE_ASR_SENTENCE_MAX_DURATION_MS = 9000;
const AUTO_FILE_RENDER_INTERVAL_MS = 100;
...
let autoFileCueFallbackTimer = null;
...
function getSentenceAwareLanguage() {
  const sourcePrefix = getLanguagePrefix(currentSourceLang);
  if (sourcePrefix === 'de' || sourcePrefix === 'ja') return sourcePrefix;
  const targetPrefix = getLanguagePrefix(currentSettings.lang);
  return targetPrefix === 'de' || targetPrefix === 'ja' ? targetPrefix : '';
}

function getSentenceAwareHardLimit() {
  const sentenceLanguage = getSentenceAwareLanguage();
  if (sentenceLanguage === 'de') return 150;
  if (sentenceLanguage === 'ja') return 75;
  return isSpaceDelimitedLang(currentSourceLang) ? 140 : 75;
}
...
function translateLiveAsrSentence(sourceText, renderSequence, generation) {
  ...
  // Keep the previous completed bilingual cue visible while translation is
  // in flight. Only after the bounded wait do we expose the source with the
  // existing automatic-caption hint.
  liveAsrTranslationFallbackTimer = setTimeout(() => {
    if (renderSequence !== liveAsrRenderSequence || generation !== trackLoadGeneration) return;
    renderLiveAsrContent(sourceText, '', true, renderSequence, generation);
  }, LIVE_ASR_TRANSLATION_WAIT_MS);

  requestTranslation(sourceText).then((translation) => {
...
function commitLiveAsrSentence(text = liveAsrPendingText) {
  ...
  const completeSentence = liveAsrSentenceBuffer;

  // For sentence-sensitive languages, hold the rolling source fragments until
  // the sentence is complete. This prevents repeated hint/source repaints.
  if (!currentSettings.showTrans) {
    const renderSequence = ++liveAsrRenderSequence;
    renderLiveAsrContent(completeSentence, '', false, renderSequence, generation);
  }

  clearTimeout(liveAsrSentenceFinalizeTimer);
  const reachedHardLimit = completeSentence.length >= getSentenceAwareHardLimit();
  const reachedDurationLimit = Date.now() - liveAsrSentenceStartedAt >= LIVE_ASR_SENTENCE_MAX_DURATION_MS;
  if (hasTerminalCaptionPunctuation(completeSentence) || reachedHardLimit || reachedDurationLimit) {
...
function feedLiveAsrSnapshot(snapshot) {
  ...
  const canRemoveCommittedPrefix = hasTerminalCaptionPunctuation(liveAsrCommittedText) ||
    (needsCompleteAutoSentence() && !liveAsrSentenceBuffer);
...
async function downloadAndParseSubtitles(trackUrl, signal, options = {}) {
  ...
  const sentenceAware = needsCompleteAutoSentence();
  const sentenceHardLimit = getSentenceAwareHardLimit();
  const sentenceMaxDuration = getSentenceAwareLanguage() === 'de' ? 10 : 9;
  let buffer = '';
  let bufferStart = -1;
  let bufferEnd = -1;
  ...
  data.events.forEach((event, eventIndex) => {
    ...
    const expandsBuffer = Boolean(
      buffer && segmentText.startsWith(buffer) && start <= bufferEnd + 0.1
    );
    const exceedsSentenceLimit = sentenceAware && buffer && !expandsBuffer &&
      buffer.length + (useSpace ? 1 : 0) + segmentText.length > sentenceHardLimit;

    const gapLimit = sentenceAware ? 1.2 : 0.85;
    if (buffer && (gap > gapLimit || (sentenceAware && startsNewSpeaker) ||
    ...
    if (sentenceAware) {
      const bufferDuration = bufferStart >= 0 ? start - bufferStart : 0;
      if (hasTerminalCaptionPunctuation(buffer) ||
          buffer.length >= sentenceHardLimit ||
          bufferDuration >= sentenceMaxDuration) {
        flushBuffer();
      }
    } else if (/[.!?。！？…]$/.test(segmentText) || buffer.length >= maxLength) {
      flushBuffer();
    }
  });
  flushBuffer();
...
function renderFileCue() {
  ...
  if (isAutoGenerated && pendingFileCueIndex >= 0 && renderedFileCueIndex >= 0 &&
    ...
    if (displayedCue && playbackTime > displayedCue.end + 0.45) {
      renderedFileCueIndex = -1;
      clearSubtitleContent();
    }
  }
  ...
  if (nextCueIndex < 0) {
    clearTimeout(autoFileCueFallbackTimer);
    autoFileCueFallbackTimer = null;
    pendingFileCueIndex = -1;
    renderedFileCueIndex = -1;
...
async function warmAutoFileTranslations(generation) {
  ...
  const warmupEnd = playhead + AUTO_FILE_WARMUP_LOOKAHEAD_SECONDS;
  const warmupTexts = [];

  for (let index = nearestIndex;
       index < preloadedSentencesList.length && warmupTexts.length < AUTO_INITIAL_TRANSLATION_LOOKAHEAD;
       index += 1) {
    const cue = preloadedSentencesList[index];
    if (warmupTexts.length && cue.start > warmupEnd) break;
    if (!preloadedTranslations.has(cue.text)) warmupTexts.push(cue.text);
  }

  if (!warmupTexts.length) return;
  await Promise.race([
    Promise.all(warmupTexts.map((text) => ensureCueTranslation(text, generation))),
...
async function preloadTranslations(generation) {
  ...
  const initialLookahead = isAutoGenerated
    ? AUTO_INITIAL_TRANSLATION_LOOKAHEAD
    : INITIAL_TRANSLATION_LOOKAHEAD;
  const initialCues = sourceSentences
    .slice(0, initialLookahead)
    .filter((text) =>
